...
On [[Sparse Graph|sparse]] **graphs**, where $m$ is small, say $O(n\;log\;n)$, then **Kruskal's** (or **Prim's** with a [[Priority Queues|priority queue]]) give better results than classic **Prim's** - $O(m\;log\;n)=O(n\;log^2\;n)$.