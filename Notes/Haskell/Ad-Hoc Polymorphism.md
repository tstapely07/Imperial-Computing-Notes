...
Another interesting method using `Ord` is `compare` which returns an [[Ordering|ordering]]:
```Haskell
compare a :: Ord a => a -> a -> Ordering
compare x y
	| x < y = LT
	| x == y = EQ
	| otherwise = GT
...
We could define the **[[Typeclasses#^5b53cc|lexicographical ordering]]** using `compare`, and a [[Case Expressions|case expression]]:
```Haskell
