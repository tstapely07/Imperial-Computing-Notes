...
* $\vec{0} \in V$ such that $\overrightarrow{0}\oplus\overrightarrow{x}=\overrightarrow{x}\oplus\overrightarrow{0}=\overrightarrow{x}$ - **additive identity**.

>[!Info]
>The **Euclidean space** $(\mathbb{R}^n,+,\times,\vec{0})$ is an obvious example of a **vector space**.
...
* Assume $\operatorname{dim}(P)=l$, where $l$ is **finite**. We can then create $l+1$ [[Linear Independence|linearly independent]] **polynomials**, which is a **contradiction**.
