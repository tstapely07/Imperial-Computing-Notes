...
To make `Ord` compatible with `Eq` we must use `Eq` in the definition of `Ord`:
```Haskell
class Eq a => Ord a where
	(<=) :: a -> a -> Bool
	-- ...
```
We can define an **instance** of `Ord` that implements the **lexicographic ordering** for a list:
```Haskell
